...
public class ColeccionNodos <T extends Comparable<T>> {
	...
	public void add(Nodo<T> NodoOrigen, Nodo<T> NodoDestino) {
		HashSet<Nodo<T>> nodo = this.data.get(NodoOrigen);
		if (nodo == null) this.data.put(NodoOrigen, nodo = new HashSet<>());
		if (!nodo.contains(NodoDestino)) nodo.add(NodoDestino);
	}
	...
	private TreeMap<Nodo<T>, TreeSet<Nodo<T>>> toOrderedCollection() {
		...
		for (Entry<Nodo<T>, HashSet<Nodo<T>>> entrada : this.data.entrySet()) {
			TreeSet<Nodo<T>> destino = new TreeSet<>(entrada.getValue());
	...
	public String toString() {
		...
		String result = "";
		for (Nodo<T> origen : aux.keySet()) {
	        result += origen + "\n";
	        for (Nodo<T> destino : aux.get(origen)) {
	            result += "\t" + destino + "\n";
	        }
	    }
		return result;
