...
public class Network<Vertex extends Comparable<Vertex>> implements Graph<Vertex> {
	...
	@Override
  	...
  	public TreeSet<Vertex> getNeighbors(Vertex v) {
  		...
  		HashMap<Vertex, Double> vecinos = adjacencyList.get(v);
  		return (vecinos == null) ? null : new TreeSet<>(vecinos.keySet());
  	}
  	...
  	public String toString() {
  		...
  		//1 for()
  		for (Entry<Vertex, HashMap<Vertex, Double>> vertice : this.adjacencyList.entrySet()) {
			resultado.put(vertice.getKey(), new TreeMap<>(vertice.getValue()));
		}
	...
	public ArrayList<Par<Vertex, Double>> getDijkstra(Vertex source, Vertex destination) {
    	...
    	for(Vertex vertice : this.adjacencyList.keySet()) {
    		if(vertice.equals(source)) source = vertice;
    		if(vertice.equals(destination)) destination = vertice;
    	}
